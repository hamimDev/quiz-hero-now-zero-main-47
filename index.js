...
//   document.location.href = "https://stellular-dango-89fa47.netlify.app/"
// })
...
const displayQuiz = (data) => {
  ...
  if (!data) {
    quizContainer.innerText = 'hi';
  }
    

    if (data.length > 6) {
    // If there are more than 6 elements, slice the array to get only the first 6
    aiS = data.slice(0, 6);

    // Remove the 'd-none' class from the 'showAll' element
    showAll.classList.remove('hidden');
    } 
    else {
    // If there are 6 or fewer elements, add the 'd-none' class to the 'showAll' element
